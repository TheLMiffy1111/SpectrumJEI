...
public class AnvilCrushingRecipeCategory extends AbstractGatedRecipeCategory<AnvilCrushingRecipe> {
	...
	public void setRecipe(IRecipeLayoutBuilder builder, AnvilCrushingRecipe recipe, IFocusGroup focuses) {
		...
		addItem(builder, RecipeIngredientRole.OUTPUT, 105, 15, recipe.getOutput(), JEIDrawables.OUTPUT_SLOT, visible);
	}
	...
	public void draw(AnvilCrushingRecipe recipe, IRecipeSlotsView recipeSlotsView, MatrixStack poseStack, double mouseX, double mouseY) {
		...
		if(isVisible(recipe)) {
			WALL.draw(poseStack, 10, 0);
			FALL.draw(poseStack, 32, 0);
			JEIDrawables.RECIPE_ARROW.draw(poseStack, 60, 15);
			...
			font.draw(poseStack, xpComponent, 126 - font.getWidth(xpComponent), 38, 0x3F3F3F);
			font.draw(poseStack, forceComponent, getWidth() / 2 - font.getWidth(forceComponent) / 2, 50, 0x3F3F3F);
		}
	}
